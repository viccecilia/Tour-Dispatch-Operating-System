...
Page({
  ...
  onShow() {
    ...
    api.setActiveTab('/package_company_lite/pages/home/index');
    ...
    return Promise.all([
      ...
      .then((results) => {
        ...
        const draftRows = (Array.isArray(dispatchDrafts && dispatchDrafts.drafts) ? dispatchDrafts.drafts : [])
          ...
          .map((item) => this.decorateUnassignedOrder({ ...item, kind: 'draft' }));
        const orderRows = (Array.isArray(unassigned && unassigned.orders) ? unassigned.orders : [])
          .filter((item) => !tenantId || !item.tenant_id || Number(item.tenant_id) === tenantId)
          .map((item) => this.decorateUnassignedOrder({ ...item, kind: 'order' }));
        ...
        const dispatchOverview = this.buildDispatchOverview(todayDispatchRows, allDrivers, allVehicles);
        const idleResources = this.buildIdleResources(todayDispatchRows, allDrivers, allVehicles);
        ...
        const panel = this.data.canDispatch
          ? this.buildDispatchPanel(this.data.panelMode, unassignedRows, notificationRows)
          : this.buildPanel(this.data.panelMode, todayRows, driverRows, pendingRows, exceptionRows, notificationRows);
        ...
        this.setData({
          ...
          unassignedRows,
          driverOrderChipLabel: this.data.canDispatch ? '待委派订单' : '今日订单',
          driverOrderChipCount: this.data.canDispatch ? unassignedRows.length : todayRows.length,
          allOrderChipCount: this.data.canDispatch ? unassignedRows.length : driverRows.length,
          driverOrderChipAlert: false,
  ...
  buildDispatchOverview(assignments, drivers, vehicles) {
    const activeStatuses = ['departed', 'arrived', 'in_service'];
    ...
    return {
      ...
      idleVehicles: Math.max(0, availableVehicles.filter((item) => !vehicleIds.has(Number(item.id || 0))).length)
    };
  },
  ...
  buildDispatchPanel(mode, unassignedRows, unread) {
    const config = {
      orders: { title: '待委派订单', hint: '未派车订单集中显示在这里，点击进入派车工作台处理。', rows: unassignedRows },
      all: { title: '待委派订单', hint: '当前公司所有未派车订单。', rows: unassignedRows },
      pending: { title: '待委派订单', hint: '请尽快安排司机和车辆。', rows: unassignedRows },
      exceptions: { title: '异常订单', hint: '异常订单请到通知或异常中心确认。', rows: [] },
      unread: { title: '未读通知', hint: '大厅成交、派车和运行提醒显示在这里。', rows: unread }
    };
    return config[mode] || config.orders;
  },
  ...
  switchPanel(e) {
    ...
    if (this.data.canDispatch) {
      const panel = this.buildDispatchPanel(mode, this.data.unassignedRows || [], this.data.notifications || []);
  ...
  onPanelRowTap(e) {
    ...
    if (item.action === 'profile') {
      wx.reLaunch({ url: '/package_company_lite/pages/profile/index' });
      return;
    }
    if (item.action === 'map') {
      this.goMap();
  ...
  goFinance() {
    ...
    wx.reLaunch({ url: '/package_company_lite/pages/finance/index' });
  },

  goMap() {
    wx.reLaunch({ url: '/package_company_lite/pages/map/index' });
  },
  ...
  goInfo(e) {
    ...
    wx.reLaunch({ url: '/package_company_lite/pages/info/index' });
  },

  goTask() {
    wx.reLaunch({ url: '/package_company_lite/pages/task/index' });
  },
