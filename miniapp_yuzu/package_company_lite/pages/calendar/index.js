...
Page({
  ...
  onShow() {
    api.setActiveTab('/package_company_lite/pages/calendar/index');
    this.refreshTabBar();
  ...
  selectDay(e) {
    ...
    const currentStart = this.data.visibleRangeStart || date;
    const currentEnd = this.data.visibleRangeEnd || date;
    const nextStart = this.isDateWithinRange(date, currentStart, currentEnd) ? currentStart : date;
    const windowState = this.buildCalendarWindowState(nextStart, this.data.mode);
    this.setData({
      selectedDate: date,
      anchorDate: date,
      displayMonth: windowState.displayMonth,
  ...
  rebuildCalendar() {
    ...
    if (this.data.mode === '30d') {
      timelineWeeks = this.buildTimelineWeeks(initialVisibleStart, selectedDate, timelineBounds);
      const anchorRowIndex = this.findWeekIndexForDate(timelineWeeks, visibleRangeStart);
      const maxRowIndex = Math.max(0, timelineWeeks.length - 5);
      const storedRowStart = this.firstDateForWeekRow(timelineWeeks, viewportStartRowIndex);
      const preferredRowIndex = storedRowStart === visibleRangeStart ? viewportStartRowIndex : anchorRowIndex;
      const safeRowIndex = Math.min(maxRowIndex, Math.max(0, preferredRowIndex));
      const safeRow = timelineWeeks[safeRowIndex] || timelineWeeks[anchorRowIndex] || timelineWeeks[0];

      if (safeRow && safeRow.cells && safeRow.cells.length) {
        const viewport = this.resolveViewportState(timelineWeeks, safeRowIndex);
        viewportStartRowIndex = safeRowIndex;
        visibleRangeStart = viewport.visibleRangeStart;
        visibleRangeEnd = viewport.visibleRangeEnd;
        displayMonth = viewport.displayMonth;
        displayYear = displayMonth.slice(0, 4);
        displayMonthLabel = displayMonth.slice(5, 7);
        rangeStart = viewport.highlightStart;
        rangeEnd = viewport.highlightEnd;
        timelineWeeks = this.annotateTimelineWeeks(timelineWeeks, viewport);
        timelineScrollTopTarget = safeRowIndex * 114;
      }
    } else {
  ...
  buildCalendarWindowState(startDateText, mode) {
    const visibleRangeStart = startDateText || this.formatDate(new Date());
    const visibleRangeEnd = mode === '7d'
      ? this.addDays(visibleRangeStart, 6)
      : this.addDays(visibleRangeStart, 34);
    const displayMonth = this.resolveDisplayMonthForWindow(visibleRangeStart, mode);
    return {
      visibleRangeStart,
      visibleRangeEnd,
      displayMonth,
