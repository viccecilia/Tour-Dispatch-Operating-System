...
const EVIDENCE_ACTIONS = {
  arrived: { type: 'arrive_waiting_photo', label: '上传照片' },
  in_service: { type: 'waypoint_photo', label: '上传照片' },
  completed: { type: 'dropoff_photo', label: '上传照片' },
  returned: { type: 'dropoff_photo', label: '上传照片' }
};
...
Page({
  data: {
    ...
    loading: false,
    submitting: false,
    uploadingEvidenceId: 0
  },

  onShow() {
    api.setActiveTab('/package_company_lite/pages/task/index');
  ...
  loadTasks() {
    ...
    Promise.all([
      ...
      .then((results) => {
        const res = results && results[0] || { assignments: [] };
  ...
  decorateAssignment(item) {
    ...
    const evidenceAction = EVIDENCE_ACTIONS[status] || null;
    return {
      ...item,
      rawStatus: status,
      routeText: `${item.pickup_location || '-'} -> ${item.dropoff_location || '-'}`,
      timeText: `${item.order_date || '-'} ${item.start_time || '--:--'}`,
      guestContactText: this.formatGuestContact(item.guest_contact),
  ...
  uploadEvidence(e) {
    const assignmentId = Number(e.currentTarget.dataset.id || 0);
    const evidenceType = String(e.currentTarget.dataset.evidenceType || '').trim();
    const item = (this.data.assignments || []).find((row) => Number(row.assignment_id || row.id || 0) === assignmentId);
    if (!assignmentId || !evidenceType || !item || !this.data.driverId || this.data.uploadingEvidenceId) return;
    wx.chooseMedia({
      count: 1,
      mediaType: ['image'],
      ...
      success: (res) => {
        ...
        this.setData({ uploadingEvidenceId: assignmentId, message: '' });
        this.readFileBase64(file.tempFilePath)
          .then((imageBase64) => api.uploadDriverEvidence({
            driver_id: this.data.driverId,
            assignment_id: assignmentId,
            evidence_type: evidenceType,
            image_base64: imageBase64,
            note: `${item.oid || item.order_id || '任务'} - ${evidenceType}`
          }))
          .then((result) => {
            this.setData({ uploadingEvidenceId: 0 });
            if (result && result.success === false) {
              wx.showToast({ title: this.evidenceError(result), icon: 'none' });
              return;
            }
  ...
  evidenceError(result) {
    return {
      invalid_evidence_request: '照片请求无效',
      assignment_not_found_for_driver: '未找到司机任务',
      evidence_status_not_allowed: '当前节点还不能上传这类照片',
      invalid_image_base64: '图片格式不正确',
      empty_image: '图片内容为空',
      image_too_large: '图片过大，请重试'
