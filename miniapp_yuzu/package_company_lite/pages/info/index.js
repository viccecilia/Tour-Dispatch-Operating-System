...
Page({
  ...
  onShow() {
    api.setActiveTab('/package_company_lite/pages/info/index');
    this.refreshTabBar();
  ...
  loadInfo() {
    ...
    Promise.all([
      ...
      const driversRes = results && results[0] || { drivers: [] };
      const vehiclesRes = results && results[1] || { vehicles: [] };
      const assignmentsRes = results && results[2] || { assignments: [] };
      const dashboard = results && results[3] || { counts: {} };
